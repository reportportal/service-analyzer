...
func TestListIndices(t *testing.T) {
	...
	for _, test := range tests {
		ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			assert.Equal(t, expectedMethod, actualMethod)
			expectedURI := "/_cat/indices?format=json"
			actualURI := r.URL.RequestURI()
			assert.Equal(t, expectedURI, actualURI)
			w.WriteHeader(test.params["statusCode"].(int))
			w.Write([]byte(test.params["response"].(string)))
		}))
		defer ts.Close()
